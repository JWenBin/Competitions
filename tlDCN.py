...
tf.app.flags.DEFINE_integer("feature_size", 12476, "Number of features")
tf.app.flags.DEFINE_integer("field_size", 14, "Number of fields")
tf.app.flags.DEFINE_integer("embedding_size", 32, "Embedding size")
tf.app.flags.DEFINE_integer("num_epochs", 10, "Number of epochs")
# tf.app.flags.DEFINE_integer("batch_size", 64, "Number of batch size")
tf.app.flags.DEFINE_integer("batch_size", 1024, "Number of batch size")
tf.app.flags.DEFINE_integer("log_steps", 1000, "save summary every steps")
tf.app.flags.DEFINE_integer("max_steps", 200000, "Int.用于训练模型的总步骤的正数.如果为None,则一直训练.训练input_fn预计不会产生OutOfRangeError或StopIteration异常.")
...
tf.app.flags.DEFINE_string("data_dir", 'F:\\python\\tf_repos-master\\traintl', "data dir")
...
def input_fn(filenames, batch_size=32, num_epochs=10, perform_shuffle=False):
...
def model_fn(features, labels, mode, params):
    ...
    Feat_Emb = tf.get_variable(name='emb', shape=[feature_size, embedding_size],
                               initializer=tf.glorot_normal_initializer())
    # print(Feat_Emb.shape, feature_size,embedding_size)
    ...
    with tf.variable_scope("Embedding-layer"):
        # print(Feat_Emb.shape,feat_ids.shape)
        embeddings = tf.nn.embedding_lookup(Feat_Emb, feat_ids)  # None * F * K
        # print('embeddings ',embeddings.shape)
        # print(feat_ids.shape)
        # print(feat_vals.shape,field_size)
...
def main(_):
    ...
    if FLAGS.task_type == 'train':
        train_spec = tf.estimator.TrainSpec(
            ...
            input_fn=lambda: input_fn(tr_files, num_epochs=FLAGS.num_epochs, batch_size=FLAGS.batch_size), max_steps= FLAGS.max_steps)
