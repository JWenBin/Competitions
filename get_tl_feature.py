...
# ("target","date","hour","minute", "uid","newsid","pos","app_version","device_vendor","netmodel","osversion","device_version","level","personidentification","gender")
continous_features = range(1, 4)
categorial_features = range(4, 15)

# Clip integer features. The clip point for each integer feature
# is derived from the 95% quantile of the total values in each feature
continous_clip = [50, 1.4]
...
class CategoryDictGenerator:
    ...
    def build(self, datafile, categorial_features, cutoff=0):
        ...
        for i in range(0, self.num_feature):
            ...
            # print('zip(*self.dicts[i])',zip(*self.dicts[i]))
            # print('list(zip(*self.dicts[i]))',list(zip(*self.dicts[i])))
...
def preprocess(datadir, outdir):
    ...
    with open(FLAGS.output_dir + 'tr.libsvm', 'w') as out_train:
        with open(FLAGS.output_dir + 'va.libsvm', 'w') as out_valid:
            with open(FLAGS.input_dir + 'train.txt', 'r',encoding='utf-8') as f:
                for line in f:
                    features = line.rstrip('\n').split(',')
    ...
    with open(FLAGS.output_dir + 'te.libsvm', 'w') as out:
        with open(FLAGS.input_dir + 'test.txt', 'r',encoding='utf-8') as f:
            for line in f:
                features = line.rstrip('\n').split(',')
                ...
                # print(len(features),len(categorial_features))
                for i in range(0, len(categorial_features)):
