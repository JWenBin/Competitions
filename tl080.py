...
# 1、对曝光到点击的时间间隔（反应时间）进行统计几乎没用，因此去掉了；
# 2、主要包含三组特征：
# （1）历史信息，即前一天的点击量、曝光量、点击率；
# （2）前x次曝光、后x次曝光到当前的时间差，后x次到当前曝光的时间差是穿越特征，并且是最强的特征；
# （3）二阶交叉特征。
# 3、该数据集缺陷很多，业务特征也很多都失效，用来作为新手入门可能不太合适。代码里一些地方加了注释，说明了上分点，可以看看；
# 4、之所以去掉了第一天的数据，有两个原因，一是因为第一组特征（历史信息）在第一天的数据上是空的，二是因为机器资源不够了。
# 5、不过历史信息特征并非主特征，因此如果机器资源充足，就不用去掉第一天数据，实在不舒服就把历史信息特征去掉试试，全数据用上应该还能提不少分。
# 6、之前用的免费服务器用不了了，只能用自己的笔记本，后面还能跑的话就继续开源，跑不动就算了；
# 7、祝各位早日登顶。
